...
int cmd_closedir(int argcnt, char* argvec[]);
...
dispatch_t dispatchTable[] = {
	...
	{"opendir", cmd_opendir, "Open a directory"},
    	{"readdir", cmd_readdir, "Read a directory entry"},
    	{"closedir", cmd_closedir, "Close a directory"}
};

static int dispatchcount = sizeof (dispatchTable) / sizeof (dispatch_t);
static int current_index = 0;  // For managing the current index in the directory listing
static DE* dir = NULL;  
...
DE* opendirectory(char* path) {
    struct ppReturn info;
    if (parsePath(path, &info) != 0) {
...
DE* readdirectory(DE* dir) {
    ...
    if (dir == NULL) {
        printf("Error: Directory handle is NULL.\n");
        return NULL;
    }
    if (current_index >= DIR_ENTRY_SIZE) {
        current_index = 0;
        return NULL; // No more entries
    }
    return &dir[current_index++];
}

void closedirectory(DE* dir) {
    // Reset the static index used in readdirectory
    current_index = 0;
}
...
int cmd_readdir(int argcnt, char* argvec[]) {
    if (argcnt != 1) {
        printf("Usage: readdir\n");
        return -1;
    }
    DE* entry = readdirectory(dir); // Assuming 'dir' is globally accessible or managed
    if (entry != NULL) {
...
int cmd_closedir(int argcnt, char* argvec[]) {
    if (argcnt != 1) {
        printf("Usage: closedir\n");
        return -1;
    }
    closedirectory(dir); // Assuming 'dir' is globally accessible or managed
